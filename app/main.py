...
def request_upload_url(
    ...
    try:
        ...
        raise HTTPException(status_code=400, detail=f"잘못된 파일 이름 형식입니다: {e}")
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"스토리지 URL 생성에 실패했습니다: {e}")
...
def register_user(user_data: user_schemas.UserCreate):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"데이터베이스 저장에 실패했습니다: {e}")
    return {"message": "회원가입이 성공적으로 완료되었습니다."}

@app.post("/api/auth/login", response_model=user_schemas.Token)
...
def generate_page(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
            detail=f"AI 서비스 호출 또는 응답 처리 중 오류가 발생했습니다: {e}"
        )
